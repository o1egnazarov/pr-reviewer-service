...
public class UserServiceDefaultImpl implements UserService {
    ...
    public UserEntity setActive(String userId, boolean isActive) {
        UserEntity userEntity = userRepository.findById(userId).orElseThrow(
                () -> new NotFoundException(ErrorCode.NOT_FOUND, "User with id: " + userId + " not found")
        );
    ...
    public List<PullRequestEntity> getReviewByUserId(String userId) {
        UserEntity user = userRepository.findWithReviewerPullRequestsById(userId).orElseThrow(
                () -> new NotFoundException(ErrorCode.NOT_FOUND, "User with id " + userId + " not found!")
        );
