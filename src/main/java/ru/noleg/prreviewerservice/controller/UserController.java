...
public class UserController implements UsersApi {
  ...
  public UserController(
      UserService userService, UserMapper userMapper, PullRequestMapper pullRequestMapper) {
  ...
  public ResponseEntity<SetIsActive200Response> setIsActive(SetIsActiveRequest setIsActiveRequest) {
    UserEntity userEntity =
        userService.setActive(setIsActiveRequest.getUserId(), setIsActiveRequest.getIsActive());
    return ResponseEntity.status(HttpStatus.OK).body(userMapper.mapToUserResponse(userEntity));
  }
}
